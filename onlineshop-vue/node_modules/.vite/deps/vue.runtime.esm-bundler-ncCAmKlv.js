...
function prepareTransitionSwitch(previous, next, props, state, instance, resumeAfterLeave, delayedLeaveSource) {
	...
	let enterHooks = resolveTransitionHooks$1(nextInner, props, state, instance, (hooks) => {
...
function prepareLeavingTransitionHooks(previousInner, props, state, instance) {
	return setTransitionHooks(previousInner, resolveTransitionHooks$1(previousInner, props, state, instance));
}
...
var useAsyncComponentState = (delay, timeout, onError, instance = currentInstance) => {
...
var isFunctionalFallthroughKey = (key) => key === "class" || key === "style" || isOn(key);
...
* @internal
*/
var restoreCurrentInstance = (prev) => {
...
/* @__NO_SIDE_EFFECTS__ */
function child(node) {
	if (isHydrating$1) return locateChildByLogicalIndex(node, 0);
	return /* @__PURE__ */ _child(node);
}
/* @__NO_SIDE_EFFECTS__ */
...
var pendingAsyncHydrationResets = 0;
var asyncHydrationIsEnabled = false;
var asyncHydrationIsHydrating = false;
var asyncHydrationNode = null;
function isInAsyncHydration() {
	return deferredHydrationBoundaryDepth > 0 || pendingAsyncHydrationResets > 0;
}
function enterAsyncHydration(node) {
	if (pendingAsyncHydrationResets++ === 0) {
		asyncHydrationIsEnabled = isHydratingEnabled$1;
		asyncHydrationIsHydrating = isHydrating$1;
		asyncHydrationNode = currentHydrationNode;
	}
	if (!isHydratingEnabled$1) setIsHydratingEnabled(true);
	setIsHydrating(true);
	...
	return () => {
		...
		if (!asyncHydrationIsEnabled) setIsHydratingEnabled(false);
	};
}
var adoptTemplate;
...
var RenderEffect = class extends ReactiveEffect {
	...
	notify() {
		if (!(this.flags & 256)) queueJob(this.job, this.i ? this.i.uid : void 0, false, this.order);
	}
};
...
function getPropsProxyHandlers(comp) {
	...
	const normalizeRawProp = (key, value) => key === "class" && value && !isString(value) ? normalizeClass(value) : value;
	const getProp = (instance, key) => {
		...
		if (dynamicSources) {
			...
			while (i--) {
				...
				for (rawKey in source) if (camelize(rawKey) === key) return resolvePropValue(propsOptions, key, normalizeRawProp(key, isDynamic ? source[rawKey] : resolveSource(source[rawKey])), instance, resolveDefault);
			}
		}
		for (const rawKey in rawProps) if (camelize(rawKey) === key) return resolvePropValue(propsOptions, key, normalizeRawProp(key, resolveSource(rawProps[rawKey])), instance, resolveDefault);
...
var shouldSkipFallthroughKey = (el, key) => {
	const instance = currentInstance;
	return !isApplyingFallthroughProps && el.$root && instance.hasFallthrough && instance.type.inheritAttrs !== false && key in instance.attrs && (!shouldUseFunctionalFallthrough(instance.type) || isFunctionalFallthroughKey(key));
};
...
var trackedScopeIdFragments = /* @__PURE__ */ new WeakMap();
...
var trackedInheritedScopeIdFragments = /* @__PURE__ */ new WeakMap();
...
function withPendingHydratingSlotBoundary(fn) {
	...
	try {
		...
		if (!completed) exitHydrationBoundary && exitHydrationBoundary();
		else if (state.pending) {
			if (state.pendingAnchors) (pendingParent.pendingAnchors || (pendingParent.pendingAnchors = [])).push(...state.pendingAnchors);
...
function resolvePendingSlotContentAnchors(state, contentValid, startIndex = 0) {
	const pendingAnchors = state.pendingAnchors;
	if (!pendingAnchors) return;
	const anchors = startIndex ? pendingAnchors.splice(startIndex) : pendingAnchors;
	if (!startIndex) state.pendingAnchors = null;
	for (let i = 0; i < anchors.length; i++) {
		const pendingAnchor = anchors[i];
		if (contentValid) pendingAnchor.onContent();
		else pendingAnchor.onFallback();
	}
}
function queuePendingSlotContentAnchor(anchor) {
	const state = currentHydratingSlotBoundaryState;
	if (state && state.pending) {
		(state.pendingAnchors || (state.pendingAnchors = [])).push(anchor);
		return true;
	}
	return false;
}
function startPendingSlotContent(start) {
	...
	const pendingAnchorStart = state.pendingAnchors ? state.pendingAnchors.length : 0;
	state.pending = true;
	let active = true;
	return (contentValid) => {
		if (!active) return;
		active = false;
		resolvePendingSlotContentAnchors(state, contentValid, !contentValid && prevPending ? pendingAnchorStart : 0);
...
function queueAnchorInsert(parentNode$1, nextNode, createAnchor, fallbackNextNode) {
	const insertAnchor = () => {
		let anchor = nextNode && /* @__PURE__ */ parentNode(nextNode) === parentNode$1 ? nextNode : null;
		if (!anchor && fallbackNextNode && /* @__PURE__ */ parentNode(fallbackNextNode) === parentNode$1) anchor = fallbackNextNode;
		parentNode$1.insertBefore(createAnchor(), anchor);
	};
	if (isInAsyncHydration()) queueJob(insertAnchor, void 0, true);
	else queuePostFlushCb(insertAnchor);
}
function isReusableAnchorCandidate(node, anchorLabel) {
	return !!node && (isComment(node, "") || isComment(node, "]") || anchorLabel !== void 0 && isComment(node, anchorLabel));
}
...
function resolveDynamicAnchor(frag, isEmpty) {
	...
	const ownsDynamicAnchor = anchorLabel !== void 0 || isNativeChildren;
	...
	if (isNativeChildren && isHydrationAnchor(currentHydrationNode) && /* @__PURE__ */ parentNode(currentHydrationNode)) return {
		kind: "reuse",
		node: currentHydrationNode
	};
	if (isEmpty) {
		if (isReusableAnchorCandidate(currentHydrationNode)) return reuseOrCreateAfterAnchor(currentHydrationNode);
		if (anchorLabel && currentHydrationNode && isComment(currentHydrationNode, "teleport anchor")) {
			const parentNode$3 = /* @__PURE__ */ parentNode(currentHydrationNode);
			if (parentNode$3) return {
				kind: "create",
				parent: parentNode$3,
		...
		if (!frag.isSlot && ownsDynamicAnchor && currentHydrationNode && !isComment(currentHydrationNode, "]")) {
			const parentNode$4 = /* @__PURE__ */ parentNode(currentHydrationNode);
			if (isNativeChildren && parentNode$4) return {
				kind: "create-cleanup",
				parent: parentNode$4,
				next: null,
				cleanupStart: currentHydrationNode,
				cleanupUntil: null,
				cleanupContainer: parentNode$4
			};
			if (parentNode$4) {
				const anchor = nextLogicalSibling(currentHydrationNode);
				if (isReusableAnchorCandidate(anchor, anchorLabel) && /* @__PURE__ */ parentNode(anchor)) return reuseOrCreateAfterAnchor(anchor, true);
				return {
					kind: "create-cleanup",
					parent: parentNode$4,
					next: anchor,
	...
	if (ownsDynamicAnchor && !isValidBlock(frag.nodes) && frag.nodes instanceof Comment && isReusableAnchorCandidate(frag.nodes, anchorLabel) && /* @__PURE__ */ parentNode(frag.nodes)) return reuseOrCreateAfterAnchor(frag.nodes, true);
	if (ownsDynamicAnchor && !isValidBlock(frag.nodes) && frag.nodes instanceof Comment && !/* @__PURE__ */ parentNode(frag.nodes) && currentHydrationNode) {
	...
	if (frag.isSlot && slotAnchor || anchorLabel === "if" && isArray(frag.nodes) && frag.nodes.length > 1) {
		const anchor = locateHydrationBoundaryClose(slotAnchor || currentHydrationNode, slotAnchor || null);
		if (isComment(anchor, "]")) return reuseOrCreateAfterAnchor(anchor);
		else throw new Error(`Failed to locate ${anchorLabel} fragment anchor. this is likely a Vue internal bug.`);
	}
	const node = findBlockBoundary(frag.nodes);
	const parentNode$2 = node.parentNode;
	const nextNode = node.nextNode;
	let fallbackNext = null;
	if (currentSlotEndAnchor && nextNode === currentHydrationNode && parentNode$2 && /* @__PURE__ */ parentNode(currentSlotEndAnchor) === parentNode$2) fallbackNext = currentSlotEndAnchor;
	return {
		kind: "create",
		parent: parentNode$2,
		next: nextNode,
...
function executeAnchorPlan(frag, plan) {
	...
	const createRuntimeAnchor = () => {
		...
		return frag.anchor = markHydrationAnchor(/* @__PURE__ */ createComment((_frag$anchorLabel = frag.anchorLabel) !== null && _frag$anchorLabel !== void 0 ? _frag$anchorLabel : ""));
	};
	try {
		switch (plan.kind) {
			case "reuse":
			...
			case "pending": {
				...
				queuePendingSlotContentAnchor({
					onContent: () => {
						...
						if (node && nodeParent === plan.parent && isReusableAnchorCandidate(node, frag.anchorLabel)) {
						...
						const anchor = node && nodeParent === plan.parent ? node : slotEnd && /* @__PURE__ */ parentNode(slotEnd) === plan.parent ? slotEnd : null;
			...
			case "create":
				...
				queueAnchorInsert(plan.parent, plan.next, createRuntimeAnchor, plan.fallbackNext);
				break;
			case "create-cleanup": {
...
var rawSlotsOwnerMap = /* @__PURE__ */ new WeakMap();
var rawSlotWrappersCache = /* @__PURE__ */ new WeakMap();
...
function createSlot(name = "default", rawProps, fallback, flags = 0) {
	...
	const sharedFallback = !!(flags & 16);
	const inheritFallback = !!(flags & 32);
	...
	if (/* @__PURE__ */ isRef(rawSlots._) && isInteropEnabled) {
		...
		fragment = instance.appContext.vapor.vdomSlot(rawSlots._, name, slotProps, instance, fallback, once, slotRoot, sharedFallback, inheritFallback, _insertionAnchor);
	} else {
		if (isHydrating$1) hydrationCursor = captureHydrationCursor();
		isCustomElementSlot = !!(instance.ce || instance.parent && isAsyncWrapper(instance.parent) && instance.parent.ce);
		const slotFragment = shouldUseSlotFragment(rawSlots, name, fallback, isCustomElementSlot) ? new SlotFragment(slotRoot, sharedFallback, inheritFallback, _insertionAnchor) : void 0;
		let dynamicFragment;
		if (slotFragment) fragment = slotFragment;
		else {
			dynamicFragment = new DynamicFragment("slot", false, false, false, void 0, _insertionAnchor);
		...
		const renderSlot = () => {
			...
			if (isCustomElementSlot) {
				...
				if (fallback) withSlotBoundary(slotFragment.slotBoundary, () => {
	...
	if (!isHydrating$1) {
		...
		if (_insertionParent && (isCustomElementSlot || !isAdoptedAnchor(fragment.anchor, _insertionAnchor))) insert(fragment, _insertionParent, _insertionAnchor);
	} else {
...
var deferBranchUpdateDuringLeave;
var removeBranchWithLeave;
var getInteropTransitionType$1;
var getInteropTransitionElement$1;
...
function registerTransitionInterop(getType, getElement) {
	getInteropTransitionType$1 = getType;
	getInteropTransitionElement$1 = getElement;
}
...
function getKeepAliveContext(instance) {
	let owner = instance;
	while (owner && owner.vapor && isAsyncWrapper(owner)) owner = owner.parent;
...
function clearSlotFallback(state) {
	...
	if (fallback) {
		...
		if (state.fallbackInserted && parentNode) remove(fallback, parentNode);
...
function disposeSlotResolution(state) {
	clearSlotFallback(state);
...
function runWithFragmentCtxOnly(fragment, fn) {
	if (currentSlotOwner === fragment.slotOwner && currentSlotBoundary === fragment.slotBoundary) return fn();
...
var DynamicFragment = class extends RenderContextFragment {
	constructor(anchorLabel, keyed = false, locate = true, trackSlotBoundary = false, onInvalid, adoptAnchor) {
		super(EMPTY_BLOCK$1);
		this.__df = true;
	...
	update(render, key = render, noScope = false) {
		...
		if (wasMounted) {
			...
			if (scope) if (this.keepAliveCtx) deferRemoval = this.keepAliveCtx.prepareBranchRemoval(this, scope);
			else scope.stop();
		...
		this.renderBranch(render, transition, parent, key, noScope, wasMounted || everUpdated && !!parent, removePrevious);
		setActiveSub(prevSub);
	...
	renderBranch(render, transition, parent, key, noScope, notifyUpdated, removePrevious) {
		this.current = key;
		if (render) {
			const keepAliveCtx = isKeepAliveEnabled ? this.keepAliveCtx : null;
			const useScope = !noScope || !!this.hasFallthroughAttrs;
			if (!keepAliveCtx) this.scope = useScope ? new EffectScope() : void 0;
			const renderBranch = () => {
				try {
			...
			keepAliveCtx ? keepAliveCtx.runBranchRender(this, renderBranch, useScope, removePrevious) : renderBranch();
			if (parent) {
...
var SlotFragment = class extends DynamicFragment {
	constructor(notifyParentBoundary = false, sharedFallback = false, inheritFallback = false, adoptAnchor) {
		super("slot", false, false, false, void 0, adoptAnchor);
		this.notifyParentBoundary = notifyParentBoundary;
		this.sharedFallback = sharedFallback;
		this.inheritFallback = inheritFallback;
		this.isSlot = true;
		this.disposed = false;
		...
		if (!isHydrating$1) this.insert = (parent, anchor, parentSuspense) => this.insertSlot(parent, anchor, parentSuspense);
	...
	get boundary() {
		return this.ownBoundary || (this.ownBoundary = {
	...
	insertSlot(parent, anchor, parentSuspense) {
		this.disposed = false;
	...
	updateContent(render, key) {
		if (key !== this.current) this.onContentInvalid.length = 0;
		this.nodes = this.content;
		this.update(render, key);
		this.content = this.nodes;
	}
	updateHydratingContent(render, key) {
		const contentStart = currentHydrationNode;
		let finish = null;
		try {
			if (this.sharedFallback || hasSlotFallback(this.boundary)) finish = startPendingSlotContent(contentStart);
	...
	updateSlot(render, fallback, key = render || fallback) {
		...
		const slotRender = render ? () => withSlotBoundary(boundary, render) : () => EMPTY_BLOCK$1;
		this.isUpdating = true;
		this.pendingRecheck = false;
		try {
			const shouldForce = prevLocalFallback !== fallback;
			if (isHydrating$1) if (this.sharedFallback || this.inheritFallback && !fallback) {
				const { contentStart, contentValid } = this.updateHydratingContent(slotRender, key);
				...
				if (exposedValid) {
					const end = contentStart && isComment(contentStart, "[") ? locateEndAnchor(contentStart) : null;
					if (end) {
						this.anchor = markHydrationAnchor(end);
						advanceHydrationNode(end);
					} else hydrateDynamicFragmentAnchor(this, !isValidBlock(this.nodes));
				} else if (this.sharedFallback) {
					var _this$anchorLabel;
					const slotEnd = getCurrentSlotEndAnchor();
					const end = contentStart && isComment(contentStart, "[") ? locateEndAnchor(contentStart) : null;
					if (end && end !== slotEnd) advanceHydrationNode(end);
					const anchor = markHydrationAnchor(/* @__PURE__ */ createComment((_this$anchorLabel = this.anchorLabel) !== null && _this$anchorLabel !== void 0 ? _this$anchorLabel : ""));
					this.anchor = anchor;
					const previous = slotEnd && slotEnd.previousSibling;
					if (previous && isComment(previous, "]")) markHydrationAnchor(previous);
					const attachContent = () => {
						const candidate = end && end !== slotEnd ? end : null;
						const insertionAnchor = candidate || (contentStart && contentStart.parentNode ? contentStart : slotEnd);
						const parent = insertionAnchor && insertionAnchor.parentNode;
						if (!parent) return;
						if (candidate) this.anchor = markHydrationAnchor(candidate);
						else parent.insertBefore(anchor, insertionAnchor);
					...
					if (!queuePendingSlotContentAnchor({
						...
						if (end && end !== slotEnd) markHydrationAnchor(end);
						queuePostFlushCb(attachContent);
					}
				} else {
					var _this$anchorLabel2;
					const anchor = this.anchor = markHydrationAnchor(/* @__PURE__ */ createComment((_this$anchorLabel2 = this.anchorLabel) !== null && _this$anchorLabel2 !== void 0 ? _this$anchorLabel2 : ""));
					const slotEnd = getCurrentSlotEndAnchor();
...
function resolveFragmentAnchor(adopt, anchorLabel) {
	if (adopt && adopt.nodeType === 8) {
		if (anchorLabel) adopt.data = anchorLabel;
		return adopt;
	}
	return anchorLabel ? /* @__PURE__ */ createComment(anchorLabel) : /* @__PURE__ */ createTextNode();
}
/**
...
*/
function isAdoptedAnchor(fragmentAnchor, insertionAnchor) {
...
function isForBlock(val) {
	return isFragment(val) && val.itemRef !== void 0;
}
...
function insert(block, parent, anchor = null, parentSuspense) {
	...
	if (isVaporComponent(block)) if (block.isMounted && !block.isDeactivated) insert(block.block, parent, anchor, parentSuspense);
...
function insertNode(block, parent, anchor = null, parentSuspense) {
	if (!isHydrating$1) {
		if (isTransitionEnabled && block instanceof Element && block.$transition && !block.$transition.disabled) performTransitionEnter(block, block.$transition, () => parent.insertBefore(block, anchor), parentSuspense);
...
function move(block, parent, anchor = null, moveType = 1, parentComponent, parentSuspense) {
	if (block instanceof Node) if (isTransitionEnabled && block instanceof Element && block.$transition && !block.$transition.disabled && moveType !== 2) if (moveType === 0) performTransitionEnter(block, block.$transition, () => parent.insertBefore(block, anchor), parentSuspense, true);
		...
		if (block.insert) block.insert(parent, anchor, parentSuspense, block.$transition, moveType);
...
function remove(block, parent) {
	...
	else if (isVaporComponent(block)) if (isSuspenseEnabled && isInteropEnabled && currentUnmountSuspense !== void 0) unmountComponent(block, parent, currentUnmountSuspense);
	else unmountComponent(block, parent);
...
function removeFragment(block, parent) {
	...
	if (onRemove) for (let i = 0; i < onRemove.length; i++) onRemove[i]();
...
function getBlockFirstNode(block) {
	if (block instanceof Node) return block;
		...
		return isValidBlock(nodes) ? getBlockFirstNode(nodes) : block.anchor || getBlockFirstNode(nodes);
	}
}
...
function createComponent(component, rawProps, rawSlots, isSingleRoot, once, appContext = currentInstance && currentInstance.appContext || emptyContext, managedMount = false, ce) {
	...
	try {
		...
		if (isInteropEnabled && appContext.vapor && !component.__vapor) {
			...
			if (!isHydrating$1) {
				if (_insertionParent) insert(frag, _insertionParent, _insertionAnchor, parentSuspense);
			} else frag.hydrate();
			return frag;
		}
		if (isTeleportEnabled && isVaporTeleport(component)) {
			...
			if (_insertionParent) onScopeDispose(() => frag.disposeTarget(), true);
		...
		if (keepAliveCtx) {
			if (rawProps && !once) rawProps = keepAliveCtx.isolatePropSources(rawProps);
			if (rawSlots && rawSlots.$) rawSlots = keepAliveCtx.isolateSlotSources(rawSlots);
		}
		...
		try {
			...
			if (isHydrating$1 && isAsyncWrapper(instance) && component.__asyncHydrate && !component.__asyncResolved) {
				...
				if (wasInOnceSlot) withOnceSlot(() => setupComponent(instance, component), false);
		...
		onScopeDispose(() => isInteropEnabled && currentUnmountSuspense !== void 0 ? unmountComponent(instance, void 0, currentUnmountSuspense) : unmountComponent(instance), true);
...
function shouldUseFunctionalFallthrough(component) {
	return isFunction(component) && !(isTransitionEnabled && isVaporTransition(component));
}
...
function createPlainElement(comp, rawProps, rawSlots, isSingleRoot, once) {
	...
	if (rawSlots) {
		...
		if (isHydrating$1) {
			...
			if (rawSlots.$ && !child) child = el.appendChild(markHydrationAnchor(/* @__PURE__ */ createComment("")));
...
function mountComponent(instance, parent, anchor) {
	if (isSuspenseEnabled && instance.suspense && instance.asyncDep && !instance.asyncResolved) {
		...
		const finishAsyncSetup = (setupResult) => {
			...
			try {
				...
				if (hydrating) withDeferredHydrationBoundary(() => {
					handleResult();
				...
				if (hydrating) {
					...
					instance.deferredHydrationBoundary = void 0;
				}
	...
	if (isKeepAliveEnabled && instance.shapeFlag & 256 && instance.a) queuePostRenderEffect(instance.a, void 0, instance.suspense);
	instance.isMounted = true;
	endMeasure(instance, `mount`);
}
function unmountComponent(instance, parentNode, parentSuspense = instance.suspense) {
	...
	if (!instance.isUnmounted && (instance.isMounted || pendingAsyncSetup)) {
		unregisterHMR(instance);
		invalidateMount(instance.m);
		invalidateMount(instance.a);
		if (instance.bum) invokeArrayFns(instance.bum);
		instance.scope.stop();
		if (instance.um) queuePostRenderEffect(instance.um, void 0, parentSuspense);
...
function unmountComponentWithParentSuspense(instance, parentNode, parentSuspense) {
...
function handleSetupResult(setupResult, component, instance) {
	...
	if (instance.hasFallthrough && component.inheritAttrs !== false && Object.keys(instance.attrs).length) {
		const getFallthroughAttrs = shouldUseFunctionalFallthrough(component) ? () => getFunctionalFallthrough(instance.attrs) : () => instance.attrs;
...
function applyFallthroughAttrs(block, instance, getFallthroughAttrs, scope) {
	...
	if (root && !hasSlotFragment) {
		...
		if (fallthroughAttrs && Object.keys(fallthroughAttrs).length && (hasSlotFragment || dynamicRoot && dynamicRoot.hasNonSingleRoot || isTeleportEnabled && containsTeleportFragment(block) || !accessedAttrs && (block instanceof Text || isArray(block) && block.length))) warnExtraneousAttributes(instance.attrs);
	}
}
...
function deferKeepAliveRenderEffects(instance) {
	...
	if (isHydrating$1) {
		...
		while (owner && owner.vapor && owner.suspense === suspense) {
			...
			while (isDynamicFragment(root) && !root.isSlot) root = root.nodes;
...
var _createApp;
var rootInstances = /* @__PURE__ */ new WeakMap();
...
var unmountApp = (app) => {
	const instance = app._instance || rootInstances.get(app);
...
function setDisplayUntracked(target, source) {
	const prevSub = setActiveSub();
	try {
...
function setDisplay(target, value, deferEnter = false, transition = void 0) {
	if (isVaporComponent(target)) return setDisplay(target.block, value, deferEnter, transition);
	if (isArray(target)) {
		if (target.length === 0) return;
		if (target.length === 1) return setDisplay(target[0], value, deferEnter, transition);
	}
	if (isFragment(target)) {
		if (isInteropEnabled && isInteropFragment(target) && target.$transition) transition = target.$transition;
...
var hydrateTransitionImpl = (suspense) => {
...
var VaporTransition = /* @__PURE__ */ decorate$1((props, { slots, expose }) => {
	...
	if (instance.rawSlots.$) {
		...
		renderEffect(() => {
			...
			const root = resolveTransitionBlock(frag.nodes, (fragment) => {
				hasStructuralRoot || (hasStructuralRoot = isStructuralTransitionFragment(fragment));
			});
			applyPendingVShows(frag.$transition, root, pendingVShows, hasStructuralRoot);
	...
	let isMounted = false;
	renderEffect(() => {
		const { hooks, root, hasStructuralRoot } = applyResolvedTransitionHooks(children, appliedHooks);
		appliedHooks = hooks;
		if (!isMounted) {
			isMounted = true;
...
function getTransitionType(block) {
	...
	if (isInteropEnabled && isFragment(block) && block.vnode) {
		const type = getInteropTransitionType$1(block.vnode);
		if (type !== void 0) return type;
	}
	return block;
}
...
function applyResolvedTransitionHooks(block, hooks) {
	if (isArray(block)) {
		...
		if (block.length === 1) block = block[0];
			...
			hooks,
			hasStructuralRoot: false
		};
	}
	if (hooks.applyGroup && (isForFragment(block) || isSlotFragment(block) || isVaporComponent(block) && isSlotFragment(block.block))) {
	...
	resolvedHooks.persisted = resolvedHooks.persisted || !hasStructuralRoot && hooks.persisted && hasVShowMarker(child);
...
function isStructuralTransitionFragment(fragment) {
	return !!(isDynamicFragment(fragment) && !isSlotFragment(fragment) && fragment.inTransition);
}
...
function resolveTransitionBlock(block, onFragment) {
	const children = [];
	collectTransitionBlocks$1(block, onFragment, children);
	return children[0];
}
function collectTransitionBlocks$1(block, onFragment, children) {
	if (block instanceof Node) {
		if (block instanceof Element) children.push(block);
	} else if (isVaporComponent(block)) collectComponentTransitionBlocks(block, onFragment, children);
	else if (isArray(block)) collectArrayTransitionBlocks(block, onFragment, children);
	else if (isFragment(block)) collectFragmentTransitionBlocks(block, onFragment, children);
}
function collectComponentTransitionBlocks(block, onFragment, children) {
	if (isAsyncWrapper(block)) {
		...
		collectTransitionBlocks$1(block.block.nodes, onFragment, children);
		inheritSingleComponentKey(children[start], block);
		return;
	}
	if (isVaporTransition(block.type)) return;
	const start = children.length;
	collectTransitionBlocks$1(block.block, onFragment, children);
	inheritSingleComponentKey(children[start], block);
}
function collectArrayTransitionBlocks(block, onFragment, children) {
	...
	for (const c of block) {
		...
		const nested = [];
		collectTransitionBlocks$1(c, onFragment, nested);
		if (hasFound) {
...
function collectFragmentTransitionBlocks(block, onFragment, children) {
	...
	collectTransitionBlocks$1(block.nodes, onFragment, children);
}
...
function getTransitionElement(block) {
	if (block instanceof Element) return block;
	if (isInteropEnabled && isFragment(block) && block.vnode) return getInteropTransitionElement$1(block.vnode);
}
...
function applyPendingVShows(hooks, root, pendingVShows, hasStructuralRoot) {
	...
	if (root) hooks.persisted = hooks.persisted || !hasStructuralRoot && pendingVShows.some((pending) => pending.target === root || resolveTransitionBlock(pending.target) === root);
	onBeforeMount(() => {
		let enterCbs;
		...
		if (enterCbs) {
			const cbs = enterCbs;
			queuePostRenderEffect(() => cbs.forEach((cb) => cb()), -1, hooks.instance.suspense);
		}
	});
}
	...
	setup(props, { slots, expose }) {
		...
		const keepAliveCtx = {
			...
			prepareBranchRemoval(frag, scope) {
				...
				const cacheKey = frag.keyed ? withCurrentCacheKey(frag.current, () => processShapeFlag(frag.nodes)) : processShapeFlag(frag.nodes);
				if (cacheKey === false) {
					scope.stop();
					return false;
				}
			...
			runBranchRender(frag, fn, useScope, removePrevious) {
				const cachedScope = useScope ? deleteScope(frag.current) : void 0;
...
var EMPTY_BLOCK = EMPTY_ARR;
var EMPTY_VNODES = EMPTY_ARR;
...
function prepareInteropSlotTransition(frag, vnode, previous, resumeAfterLeave, delayedLeaveSource) {
	...
	if (!transition && !(instance && isVaporTransition(instance.type))) return;
...
function unmountVaporInteropWithParentSuspense(vnode, doRemove, parentSuspense) {
	const prev = setCurrentUnmountSuspense(parentSuspense);
	try {
...
function createVNodeFragment(vnode) {
	...
	frag.$key = vnode.key;
	let contentResolved = isHydrating$1;
	const syncNodes = () => {
		frag.nodes = resolveVNodeNodes(vnode);
		contentResolved = true;
	};
	frag.isBlockValid = (componentAsValid) => contentResolved ? isValidBlock(frag.nodes, componentAsValid) : true;
...
function resolveUnmountSuspense(suspense) {
	return currentUnmountSuspense === void 0 ? suspense : currentUnmountSuspense;
}
/**
* Mount VNode in vapor
*/
function mountVNode(internals, vnode, parentComponent) {
	let suspense = parentSuspense || parentComponent && parentComponent.suspense;
	...
	const unmount = (parentNode, transition) => {
		...
		} else internals.um(vnode, parentComponent, parentSuspense, !!parentNode);
	...
	frag.insert = (parentNode, anchor, parentSuspense, transition, moveType = 2) => {
		if (isHydrating$1) return;
		...
		if (vnode.shapeFlag & 512) {
			...
			if (!isMounted) {
				if (transition) setTransitionHooks(vnode, transition);
				internals.p(null, vnode, parentNode, anchor, parentComponent, operationSuspense, void 0, vnode.slotScopeIds);
				onScopeDispose(unmount, true);
				isMounted = true;
			} else {
...
function createVDOMComponent(internals, component, parentComponent, rawProps, rawSlots, once) {
	...
	frag.insert = (parentNode, anchor, parentSuspense, transition, moveType = 2) => {
		...
		if (parentSuspense !== void 0) suspense = parentSuspense;
		const operationSuspense = suspense;
		if (vnode.shapeFlag & 512) activate(vnode, parentNode, anchor, internals, parentComponent, operationSuspense, void 0, false);
		else {
			const prev = currentInstance;
			simpleSetCurrentInstance(parentComponent);
			if (!isMounted) {
				if (transition) setTransitionHooks(vnode, transition);
				internals.mt(vnode, parentNode, anchor, parentComponent, operationSuspense, void 0, false);
...
*/
function renderVDOMSlot(internals, slotsRef, name, props, parentComponent, fallback, once, slotRoot, sharedFallback, inheritFallback, adoptAnchor) {
	let suspense = parentSuspense || parentComponent.suspense;
	...
	const cleanupInvalidContent = () => {
		...
		if (currentParentNode) removeAttachedNodes(contentState.nodes, currentParentNode);
	};
	const onContentInvalid = [cleanupInvalidContent];
	frag.isBlockValid = (componentAsValid) => {
		if (!contentResolved) return true;
		return slotResolutionState.activeFallback ? isValidBlock(slotResolutionState.activeFallback, componentAsValid) : contentState.valid;
	};
	const boundary = {
	...
	slotResolutionState = {
		...
		notifyExposedValidityChange: () => {
			if (slotRoot && !isContentUpdateRecheck && slotBoundary) slotBoundary.markDirty();
		}
	};
	if (sharedFallback && slotBoundary) registerContentInvalid(slotBoundary, () => {
	...
	if (slotRoot) trackSlotBoundaryDirtying(frag, sharedFallback ? void 0 : cleanupInvalidContent);
	...
	const setRenderedContent = (rendered, knownValid) => {
		...
		if (isVNode(rendered)) {
			...
			contentState.valid = knownValid === void 0 ? isValidSlot(rendered) : knownValid;
		} else {
	...
	const recheckResolutionAfterContentUpdate = (forceResolutionRecheck = false) => {
	...
	const finishContentUpdate = (forceResolutionRecheck = false) => {
		recheckResolutionAfterContentUpdate(forceResolutionRecheck);
		notifyUpdated();
	};
	const patchSlotVNode = (previous, next, slotScopeIds, valid) => {
		frag.vnode = next;
		...
		internals.p(previous, next, currentParentNode, currentAnchor, parentComponent, suspense, void 0, slotScopeIds);
		setRenderedContent(next, valid);
		finishContentUpdate();
	};
	const removeRenderedContent = (rendered, parentNode) => {
		const contentDetached = !!slotResolutionState.activeFallback;
		if (isVNode(rendered)) internals.um(rendered, parentComponent, resolveUnmountSuspense(suspense), !contentDetached && !!parentNode);
		else remove(rendered, contentDetached ? void 0 : parentNode);
	};
	const resumeOutIn = () => {
		queuePostFlushCb(() => {
			...
			if (isVNode(content)) {
				const nextVNode = prepareInteropSlotTransition(frag, content, void 0, NOOP) || content;
				patchSlotVNode(pending.placeholder, nextVNode, content.slotScopeIds, pending.contentValid);
			} else {
				frag.vnode = null;
				frag.$key = void 0;
				if (pending.placeholder) removeRenderedContent(pending.placeholder, currentParentNode);
				if (content) insert(content, currentParentNode, currentAnchor, suspense);
	...
	frag.insert = (parentNode, anchor, parentSuspense) => {
		...
		if (!isMounted) {
			...
			} else if (isVNode(contentState.rendered)) internals.m(contentState.rendered, parentNode, anchor, 2, parentComponent, suspense);
			else if (contentState.rendered) insert(contentState.rendered, parentNode, anchor, suspense);
			if (!sharedContentParked) insertActiveSlotFallback(slotResolutionState);
		}
		notifyUpdated();
	};
	frag.remove = (parentNode) => {
		...
		if (leave) leave(true);
		if (contentState.rendered) removeRenderedContent(contentState.rendered, storage || parentNode);
	...
	const render = () => {
		...
		try {
			const renderSlotContent = () => {
				notifyBeforeUpdate();
				runWithFragmentCtxOnly(frag, () => withSlotBoundary(boundary, () => {
					let slotContent;
					...
					if (isHydrating$1) {
						...
						const contentStart = currentHydrationNode;
						const contentEnd = contentStart && isComment(contentStart, "[") ? locateEndAnchor(contentStart) : null;
						const slotEnd = getCurrentSlotEndAnchor();
						...
						if (isVNode(hydratedContent)) {
							const hydrationVNode = prepareInteropSlotTransition(frag, hydratedContent, void 0, NOOP) || hydratedContent;
							...
							if (!hydrateForwardedEmptySlotFragment(hydrationVNode, parentComponent, slotContentValid)) hydrateVNode(hydrationVNode, parentComponent, hydrationVNode === hydratedContent ? null : hydratedContent.slotScopeIds);
						...
						if (sharedFallback && exposedValid && candidateEnd && currentHydrationNode === candidateEnd) advanceHydrationNode(candidateEnd);
							...
							const anchor = /* @__PURE__ */ createTextNode();
							...
							if (previous && isComment(previous, "]")) markHydrationAnchor(previous);
							const attachAnchor = () => {
								const insertionAnchor = candidateEnd || (contentStart && contentStart.parentNode ? contentStart : slotEnd);
								const parent = insertionAnchor && insertionAnchor.parentNode;
								if (parent) {
									if (candidateEnd) {
										frag.anchor = currentAnchor = markHydrationAnchor(candidateEnd);
										if (currentHydrationNode === contentStart) advanceHydrationNode(candidateEnd);
									} else parent.insertBefore(anchor, insertionAnchor);
					...
					if (isVNode(slotContent)) {
						...
						if (slotResolutionState.activeFallback && slotContentValid && transition) {
							if (mode === "out-in") {
								...
								const leavingElement = leavingBlock && getTransitionElement(leavingBlock);
								pendingOutIn = {
									content: slotContent,
									placeholder: null,
									contentValid: true,
						...
						const prevVNode = prevIsVNode && (!slotResolutionState.activeFallback || contentState.valid) ? prevRendered : null;
						if (prevVNode && !slotContentValid && transition && hasSlotFallback(boundary)) {
							if (mode === "out-in") {
								const placeholder = prepareInteropSlotTransition(frag, slotContent, void 0, NOOP) || createCommentVNode();
								if (prepareTransitionLeave(prevVNode, void 0, transition.props, transition.state, transition.instance, resumeOutIn)) {
									...
									internals.p(prevVNode, placeholder, currentParentNode, currentAnchor, parentComponent, suspense, void 0, null);
									return;
								}
							} else if (mode === "in-out") {
						...
						const transitionChild = prepareInteropSlotTransition(frag, slotContent, prevVNode || void 0, resumeOutIn, delayedLeaveSource);
						...
						if (prevVNode && transitionChild && transition && transition.state.isLeaving) {
							...
							internals.p(prevVNode, nextVNode, currentParentNode, currentAnchor, parentComponent, suspense, void 0, null);
							return;
						}
					...
					if (contentState.rendered) removeRenderedContent(contentState.rendered, currentParentNode);
					frag.vnode = null;
					frag.$key = void 0;
					setRenderedContent(null);
					finishContentUpdate();
	...
	frag.hydrate = () => {
		...
		if (contentState.valid && fallback && !inheritFallback && !frag.anchor) queueAnchorInsert(currentParentNode, currentAnchor, () => {
			return frag.anchor = currentAnchor = markHydrationAnchor(/* @__PURE__ */ createTextNode());
		});
		isMounted = true;
	};
	return frag;
...
var vaporInteropPlugin = (app) => {
	enableSuspense();
	registerTransitionInterop(getInteropTransitionType, getInteropTransitionElement);
	setInteropEnabled();
...
var renderEmptyVNodes = () => EMPTY_VNODES;
...
function renderVaporSlot(vnode, parentComponent, parentSuspense) {
	...
	try {
		...
		const onContentInvalid = [() => {
			if (currentParentNode) removeAttachedNodes(contentNodes, currentParentNode);
		}];
		...
		try {
			...
			slotResolutionState.pendingRecheckForce = false;
			let finish;
			const finishHydratingContent = (contentValid) => {
				if (!finish) return;
				finish(contentValid);
				finish = void 0;
			};
			...
			try {
				if (isHydrating$1) resolvedContent = withHydratingSlotBoundary(() => {
					if (hasSlotFallback(localFallbackBoundary)) finish = startPendingSlotContent(currentHydrationNode);
					try {
						return finalizeResolvedContent(runWithFragmentCtxOnly(frag, () => {
				...
				else resolvedContent = finalizeResolvedContent(runWithFragmentCtxOnly(frag, () => withSlotBoundary(localFallbackBoundary, () => invokeVaporSlot(vnode))));
			} finally {
...
function createVNodeChildrenFragment(internals, render, parentComponent) {
	...
	const notifyBeforeUpdate = () => {
		if (isMounted && frag.onBeforeUpdate) frag.onBeforeUpdate.forEach((hook) => hook());
	};
	const renderContent = () => {
		const prev = currentInstance;
		simpleSetCurrentInstance(parentComponent);
			...
			renderEffect(() => {
				runWithFragmentCtxOnly(frag, () => {
					...
					if (isHydrating$1) {
						...
						currentAnchor = currentHydrationNode;
						if (frag.slotBoundary && currentAnchor && isHydrationAnchor(currentAnchor) && currentAnchor !== getCurrentSlotEndAnchor() && currentAnchor.nextSibling) currentAnchor = currentAnchor.nextSibling;
					} else if (!isMounted) {
						currentChildren = nextChildren;
						currentVNode = createVNode(Fragment, null, nextChildren);
	...
	frag.insert = (parentNode, anchor, parentSuspense) => {
		...
		if (!isMounted) {
			...
			internals.m(vnode, parentNode, anchor, 2, parentComponent, suspense);
		});
	};
	frag.remove = (parentNode) => {
		scope.stop();
...
var interopSlotCache = /* @__PURE__ */ new WeakMap();
...
var isInternalSlotKey = (key) => key === "_" || key === "_ctx" || key === "$stable" || key === "$";
...
var interopScopeIdRootMap = /* @__PURE__ */ new WeakMap();
var interopScopeIdFragmentMap = /* @__PURE__ */ new WeakMap();
function trackInteropScopeIdFragment(instance, frag) {
	...
	(frag.onUpdated || (frag.onUpdated = [])).push(() => syncInteropRoot(instance));
}
...
var TeleportFragment = class extends RenderContextFragment {
	...
	initChildren() {
		...
		try {
			...
			renderEffect(() => runWithFragmentCtxOnly(this, () => this.handleChildrenUpdate(this.rawSlots && this.rawSlots.default ? this.rawSlots.default() : [])));
	...
	queueTargetUpdate() {
		...
		queuePostRenderEffect(this.mountToTargetJob, void 0, this.parentSuspense || null);
	}
	mountToTarget() {
	...
	*/
	_updateFragmentNodes(block, replacements) {
		...
		if (nodes instanceof HTMLSlotElement) {
			...
			if (replacement.usedFallback && isSlotFragment(block) && block.customElementFallback) {
	...
	return () => {
		if (isHydrating$1) {
			...
			if (isStatic && !isComment(currentHydrationNode, "")) {
				...
				if (html !== "") {
					if (html[0] !== "<") {
						if (!hydrateTextNode(adopted, html) && true) validateHydrationTarget(adopted, html);
					} else validateHydrationTarget(adopted, html);
				}
...
function createIf(condition, b1, b2, flags = 1) {
	...
	if (flags & 16) {
		...
		const dynamicFragment = new DynamicFragment("if", keyed, false, trackSlotBoundary, trackSlotBoundary ? () => {
	...
	if (!isHydrating$1) {
		if (_insertionParent && !adopted) insert(frag, _insertionParent, _insertionAnchor);
	} else {
...
function createKeyedFragment(key, render, trackSlotBoundary = false) {
	...
	const frag = new DynamicFragment("keyed", true, true, trackSlotBoundary, trackSlotBoundary ? () => {
		const parent = frag.anchor.parentNode;
		if (parent) removeNode(frag.anchor, parent);
	} : void 0, _insertionAnchor);
	renderEffect(() => frag.update(render, key()));
	if (!isHydrating$1) {
		if (_insertionParent && !isAdoptedAnchor(frag.anchor, _insertionAnchor)) insert(frag, _insertionParent, _insertionAnchor);
	} else exitHydrationCursor(hydrationCursor);
	return frag;
}
...
var createFor = (src, renderItem, getKey, flags = 0) => {
	...
	const renderList = () => {
		...
		if (!wasMounted) {
			...
			if (!oldLength) for (let i = 0; i < newLength; i++) mount(source, i);
				...
				if (opers.length === mountCounter) for (const { source, index, item, key } of opers) mount(source, index, index < newLength - 1 ? getBlockFirstNode(newBlocks[index + 1].nodes) : parentAnchor, item, key);
					...
					for (const action of opers) {
						...
						if (index < newLength - 1) {
							...
							if (!anchorNode.parentNode) anchorNode = getBlockFirstNode(nextBlock.nodes);
							if ("source" in action) {
							...
							let anchorNode = anchor ? getBlockFirstNode(anchor.nodes) : parentAnchor;
	...
	if (flags & 4) renderList();
		...
		const prevBoundary = restoreBoundary ? setCurrentSlotBoundary(slotBoundary) : null;
		try {
			renderList();
		} finally {
	...
	if (!isHydrating$1) {
		if (_insertionParent && !isAdoptedAnchor(parentAnchor, _insertionAnchor)) insert(frag, _insertionParent, _insertionAnchor);
	} else {
...
function setTemplateRefWithState(instance, el, state, ref, refFor, refKey) {
	...
	if (frag && state.registeredFrag !== frag) {
		...
		(frag.onUpdated || (frag.onUpdated = [])).push(() => {
			...
			state.oldRef = setRef$1(instance, state.suspense, el, state.ref, state.oldRef, state.refFor, state.refKey, state.oldRefKey);
			state.oldRefKey = state.oldRef != null ? state.refKey : void 0;
		});
	}
	const oldRef = setRef$1(instance, state.suspense, el, ref, state.oldRef, refFor, refKey, state.oldRefKey);
	state.oldRef = oldRef;
...
function setStaticTemplateRef(el, ref, refFor, refKey) {
	...
	const oldRef = setRef$1(instance, suspense, el, ref, void 0, refFor, refKey);
	const frag = getTemplateRefUpdateFragment(el);
	if (frag) (frag.onUpdated || (frag.onUpdated = [])).push(() => {
		if (isVaporComponent(el) && el.isDeactivated) return;
		setRef$1(instance, suspense, el, ref, oldRef, refFor, refKey);
	});
	return oldRef;
}
...
function createDynamicComponent(getter, rawProps, rawSlots, flags = 0) {
	...
	if (!isHydrating$1) resetInsertionState();
	const hydrationCursor = isHydrating$1 ? captureHydrationCursor() : null;
	const frag = new DynamicFragment("dynamic-component", false, true, slotRoot, slotRoot ? () => {
...
function withVaporDirectives(node, dirs) {
	...
	function trackRootUpdates(block) {
		if (isVaporComponent(block)) {
			if (block.asyncDep && !block.asyncResolved) {
			...
			return isAsyncWrapper(block) && isFragment(innerBlock) && innerBlock.nodes === EMPTY_BLOCK$1;
		}
		if (isArray(block)) {
			let hasPendingTarget = false;
			for (const child of block) if (trackRootUpdates(child)) hasPendingTarget = true;
			return hasPendingTarget;
		}
		if (!isFragment(block)) return false;
		if (!trackedBlocks.has(block)) {
			trackedBlocks.add(block);
			(block.onUpdated || (block.onUpdated = [])).push(applyDirectives);
		}
		return isInteropEnabled && isInteropFragment(block) && block.nodes === EMPTY_BLOCK$1 || trackRootUpdates(block.nodes);
	}
	onScopeDispose(() => {
		disposed = true;
		stopDirectiveScope();
...
var transitionGroupUpdateOwnerMap = /* @__PURE__ */ new WeakMap();
...
var inheritedTransitionKeyMap = /* @__PURE__ */ new WeakMap();
var transitionKeyGeneration = 0;
var currentTransitionKeyGeneration = 0;
...
function collectTransitionBlocks(block, children, onFragment, onUpdateOwner) {
	if (block instanceof Node) {
		...
		const start = children.length;
		collectTransitionBlocks(block.block, children, onFragment, isRootSlot ? onUpdateOwner : void 0);
		if (!isRootSlot) for (let i = start; i < children.length; i++) setTransitionType(children[i], block.type);
		inheritTransitionKey(children, start, block.$key);
	} else if (isArray(block)) for (let i = 0; i < block.length; i++) collectTransitionBlocks(block[i], children, onFragment, onUpdateOwner);
	else if (isFragment(block)) {
		if (onFragment) onFragment(block);
		...
		if (isInteropEnabled && block.vnode) children.push(block);
			...
			if (isForBlock(block)) {
				...
				for (let i = start; i < children.length; i++) children[i].$key = block.key != null && count > 1 ? `${block.key}:${i - start}` : block.key;
			} else inheritTransitionKey(children, start, block.$key);
		}
	}
}
...
function applyGroupTransitionHooks(block, props, state, instance, updateHooks) {
	...
	for (let i = 0; i < children.length; i++) {
		...
		if (isValidTransitionBlock(child)) {
			if (child.$key != null) child.$transition = resolveTransitionHooks(child, props, state, instance);
...
function trackTransitionGroupUpdate(owner, updateHooks) {
	...
	if (isFragment(owner)) {
		...
		owner.scope.run(() => {
			...
			effect.notify = () => {
				if (effect.flags & 256 || !effect.dirty) return;
				effect.run();
				if (isPending) return;
				isPending = true;
...
//#endregion
export { setDOMProp as $, getCurrentWatcher as $a, resolvePropValue as $i, createHydrationRenderer as $n, isSetEqual as $r, useHost as $t, createVaporApp as A, validateComponentName as Aa, openBlock as Ai, Static as An, guardReactiveProps as Ar, defineSSRCustomElement as At, insert as B, withCtx as Ba, pushWarningContext as Bi, callWithAsyncErrorHandling as Bn, inject as Br, normalizeContainer as Bt, createInvoker as C, useId as Ca, onErrorCaptured as Ci, ErrorTypeStrings as Cn, capitalize as Co, flushOnAppMount as Cr, VueElementBase as Ct, createSlot as D, useSlots as Da, onServerPrefetch as Di, MoveType as Dn, normalizeStyle as Do, getFunctionalFallthrough as Dr, createApp as Dt, createSelector as E, useSSRContext as Ea, onRenderTriggered as Ei, MismatchTypes as En, normalizeProps as Eo, getCurrentInstance as Er, callPendingCbs as Et, defineVaporSSRCustomElement as F, warnPropMismatch as Fa, popWarningContext as Fi, activate as Fn, hydrateOnInteraction as Fr, handleMovedChildren as Ft, on as G, EffectScope as Ga, registerRuntimeCompiler as Gi, computed as Gn, isFunctionalFallthroughKey as Gr, resolveTransitionProps as Gt, isVaporComponent as H, withDirectives as Ha, queuePostFlushCb as Hi, checkTransitionMode as Hn, invokeDirectiveHook as Hr, patchProp as Ht, delegate as I, watch as Ia, prepareTransitionLeave as Ii, assertNumber as In, hydrateOnMediaQuery as Ir, hasCSSTransform as It, renderEffect as J, TriggerOpTypes as Ja, resetShapeFlag as Ji, createBaseVNode as Jn, isKeepAlive as Jr, shouldSetAsPropForVueCE as Jt, onBinding as K, ReactiveEffect as Ka, renderList as Ki, createAppAPI as Kn, isHydrating as Kr, setVarsOnNode as Kt, delegateEvents as L, watchEffect as La, prepareTransitionSwitch as Li, baseEmit as Ln, hydrateOnVisible as Lr, hydrate as Lt, defineVaporAsyncComponent as M, version as Ma, performTransitionEnter as Mi, Teleport as Mn, handleError as Mr, ensureRenderer as Mt, defineVaporComponent as N, warn as Na, performTransitionLeave as Ni, Text$1 as Nn, hasInjectionContext as Nr, forceReflow as Nt, createTemplateRefSetter as O, useTemplateRef as Oa, onUnmounted as Oi, NULL_DYNAMIC_COMPONENT as On, toDisplayString as Oo, getInheritedScopeIds as Or, createSSRApp as Ot, defineVaporCustomElement as P, warnExtraneousAttributes as Pa, popScopeId as Pi, VaporSlot as Pn, hydrateOnIdle as Pr, getValue as Pt, setClassName as Q, getCurrentScope as Qa, resolveFilter as Qi, createElementBlock as Qn, isRuntimeOnly as Qr, useCssVars as Qt, getDefaultValue as R, watchPostEffect as Ra, provide as Ri, baseNormalizePropsOptions as Rn, initCustomFormatter as Rr, initDirectivesForSSR as Rt, createIf as S, useAttrs as Sa, onDeactivated as Si, ErrorCodes as Sn, camelize as So, expose as Sr, VueElement as St, createPlainElement as T, useModel as Ta, onRenderTracked as Ti, KeepAlive as Tn, normalizeClass as To, getComponentName as Tr, baseUseCssVars as Tt, next as U, withMemo as Ua, queuePostRenderEffect as Ui, cloneVNode as Un, isAsyncWrapper as Ur, patchStyle as Ut, isFragment as V, withDefaults as Va, queueJob as Vi, callWithErrorHandling as Vn, invalidateMount as Vr, parseEventName as Vt, nthChild as W, withScopeId as Wa, registerHMR as Wi, compatUtils as Wn, isEmitListener as Wr, render as Wt, setBlockKey as X, effect as Xa, resolveDirective as Xi, createCanSetSetupRefChecker as Xn, isMemoSame as Xr, unsafeToTrustedHTML as Xt, setAttr as Y, customRef as Ya, resolveComponent as Yi, createBlock as Yn, isMapEqual as Yr, svgNS as Yt, setClass as Z, effectScope as Za, resolveDynamicComponent as Zi, createCommentVNode as Zn, isMismatchAllowed as Zr, useCssModule as Zt, createComponent as _, toHandlers as _a, normalizeVNode as _i, xlinkNS as _n, toRef as _o, devtools as _r, withVaporKeys as _t, VaporKeepAlive as a, setCurrentInstance as aa, isValidHtmlOrSvgAttribute as ai, vModelRadio as an, markRaw as ao, createTextVNode as ar, setProp as at, createFor as b, unregisterHMR as ba, onBeforeUnmount as bi, Comment$1 as bn, triggerRef as bo, ensureValidVNode as br, TransitionGroup as bt, VaporTransitionGroup as c, setIsHydratingEnabled$1 as ca, logMismatchError as ci, vModelSetSelected as cn, proxyRefs as co, deactivate as cr, setTemplateRefBinding as ct, applyRadioModel as d, shouldUpdateComponent as da, mergeDefaults as di, vModelTextUpdate as dn, ref as do, defineEmits as dr, template as dt, resolveTarget as ea, isTeleportDeferred as ei, useShadowRoot as en, isProxy as eo, createInternalObject as er, setDynamicEvents as et, applySelectModel as f, simpleSetCurrentInstance as fa, mergeModels as fi, vShow as fn, shallowReactive as fo, defineExpose as fr, txt as ft, createAssetComponent as g, toClassSet as ga, normalizeRef as gi, withModifiers as gn, toRaw as go, defineSlots as gr, withVaporDirectives as gt, child as h, startMeasure as ha, nextUid as hi, withKeys as hn, stop as ho, defineProps as hr, withAsyncContext as ht, VaporFragment as i, setBlockTracking as ia, isVNode as ii, vModelDynamic as in, isShallow as io, createStaticVNode as ir, setInsertionState as it, createVaporSSRApp as j, validateProps as ja, performAsyncHydrate as ji, Suspense as jn, h as jr, ensureHydrationRenderer as jt, createTextNode as k, useTransitionState as ka, onUpdated as ki, SchedulerJobFlags as kn, toHandlerKey as ko, getTransitionRawChildren as kr, defineCustomElement as kt, applyCheckboxModel as l, setRef as la, markAsyncBoundary as li, vModelText as ln, reactive as lo, defineAsyncComponent as lr, setText as lt, applyVShow as m, ssrUtils as ma, nextTick as mi, vShowOriginalDisplay as mn, shallowRef as mo, defineOptions as mr, vaporInteropPlugin as mt, DynamicFragment as n, resolveTransitionHooks$1 as na, isTemplateNode as ni, vModelCheckboxInit as nn, isReadonly as no, createRenderer as nr, setElementText as nt, VaporTeleport as o, setCurrentRenderingInstance as oa, knownTemplateRefs as oi, vModelSelect as on, onScopeDispose as oo, createVNode as or, setStaticTemplateRef as ot, applyTextModel as p, ssrContextKey as pa, mergeProps as pi, vShowHidden as pn, shallowReadonly as po, defineModel as pr, useVaporCssVars as pt, remove as q, TrackOpTypes as qa, renderSlot as qi, createAsyncComponentContext as qn, isHydratingEnabled as qr, shouldSetAsProp as qt, VaporElement as r, restoreCurrentInstance as ra, isTemplateRefKey as ri, vModelCheckboxUpdate as rn, isRef as ro, createSlots as rr, setHtml as rt, VaporTransition as s, setDevtoolsHook as sa, leaveCbKey as si, vModelSelectInit as sn, onWatcherCleanup as so, currentInstance as sr, setStyle as st, compile as t, resolveTransitionChild as ta, isTeleportDisabled as ti, vModelCheckbox as tn, isReactive as to, createPropsRestProxy as tr, setDynamicProps as tt, applyDynamicModel as u, setTransitionHooks as ua, matches as ui, vModelTextInit as un, readonly as uo, defineComponent as ur, setValue as ut, createComponentWithFallback as v, toStyleMap as va, onActivated as vi, BaseTransition as vn, toRefs as vo, devtoolsComponentAdded as vr, withVaporModifiers as vt, createKeyedFragment as w, useInstanceOption as wa, onMounted as wi, Fragment as wn, extend as wo, getAttributeMismatch as wr, baseApplyTranslation as wt, createForSlots as x, useAsyncComponentState as xa, onBeforeUpdate as xi, DeprecationTypes as xn, unref as xo, ensureVaporSlotFallback as xr, TransitionPropsValidators as xt, createDynamicComponent as y, transformVNodeArgs as ya, onBeforeMount as yi, BaseTransitionPropsValidators as yn, toValue as yo, endMeasure as yr, Transition as yt, getRestElement as z, watchSyncEffect as za, pushScopeId as zi, baseResolveTransitionHooks as zn, initFeatureFlags as zr, nodeOps as zt };
