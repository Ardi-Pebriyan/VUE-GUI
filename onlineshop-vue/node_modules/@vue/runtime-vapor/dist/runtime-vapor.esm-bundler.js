...
function isolatePropSources(rawProps) {
	...
	for (const key in rawProps) {
		...
		if (isFunction(source)) {
			const target = committed || (committed = shallowReactive$1({}));
	...
	if (dynamicSources) {
		...
		for (let i = 0; i < dynamicSources.length; i++) {
			const source = dynamicSources[i];
			if (isFunction(source)) {
				const target = committedDynamicSources[i] = shallowReactive$1({});
				...
				for (const key in source) {
					...
					if (isFunction(value)) {
						if (!target) target = committedDynamicSources[i] = shallowReactive$1({});
...
function resolveDynamicAnchor(frag, isEmpty) {
	...
	if (frag.isSlot && slotAnchor || anchorLabel === "if" && isArray(frag.nodes) && frag.nodes.length > 1) {
		...
		else if (!!(process.env.NODE_ENV !== "production")) throw new Error(`Failed to locate ${anchorLabel} fragment anchor. this is likely a Vue internal bug.`);
	}
...
function executeAnchorPlan(frag, plan) {
	...
	const createRuntimeAnchor = () => {
		...
		return frag.anchor = markHydrationAnchor(!!(process.env.NODE_ENV !== "production") ? /* @__PURE__ */ createComment((_frag$anchorLabel = frag.anchorLabel) !== null && _frag$anchorLabel !== void 0 ? _frag$anchorLabel : "") : /* @__PURE__ */ createTextNode());
	};
	try {
		switch (plan.kind) {
			case "reuse":
...
function createSlot(name = "default", rawProps, fallback, flags = 0) {
	...
	if (isRef(rawSlots._) && isInteropEnabled) {
		...
		if (slotFragment) fragment = slotFragment;
			...
			dynamicFragment = new DynamicFragment(!!(process.env.NODE_ENV !== "production") ? "slot" : void 0, false, false, false, void 0, _insertionAnchor);
...
let getInteropTransitionType$1;
let getInteropTransitionElement$1;
...
var DynamicFragment = class extends RenderContextFragment {
	...
	renderBranch(render, transition, parent, key, noScope, notifyUpdated, removePrevious) {
		...
		if (render) {
			...
			if (!keepAliveCtx) this.scope = useScope ? new EffectScope$1() : void 0;
			const renderBranch = () => {
				try {
...
var SlotFragment = class extends DynamicFragment {
	constructor(notifyParentBoundary = false, sharedFallback = false, inheritFallback = false, adoptAnchor) {
		super(isHydrating$1 || !!(process.env.NODE_ENV !== "production") ? "slot" : void 0, false, false, false, void 0, adoptAnchor);
		...
		try {
			...
			if (isHydrating$1) if (this.sharedFallback || this.inheritFallback && !fallback) {
				...
				if (exposedValid) {
					...
					const anchor = markHydrationAnchor(!!(process.env.NODE_ENV !== "production") ? /* @__PURE__ */ createComment((_this$anchorLabel = this.anchorLabel) !== null && _this$anchorLabel !== void 0 ? _this$anchorLabel : "") : /* @__PURE__ */ createTextNode());
					this.anchor = anchor;
					...
					const anchor = this.anchor = markHydrationAnchor(!!(process.env.NODE_ENV !== "production") ? /* @__PURE__ */ createComment((_this$anchorLabel2 = this.anchorLabel) !== null && _this$anchorLabel2 !== void 0 ? _this$anchorLabel2 : "") : /* @__PURE__ */ createTextNode());
					const slotEnd = getCurrentSlotEndAnchor();
...
function resolveFragmentAnchor(adopt, anchorLabel) {
	...
	return !!(process.env.NODE_ENV !== "production") && anchorLabel ? /* @__PURE__ */ createComment(anchorLabel) : /* @__PURE__ */ createTextNode();
}
/**
...
function createComponent(component, rawProps, rawSlots, isSingleRoot, once, appContext = currentInstance && currentInstance.appContext || emptyContext, managedMount = false, ce) {
	...
	try {
		...
		if (isTeleportEnabled && isVaporTeleport(component)) {
			...
			if (_insertionParent) onScopeDispose$1(() => frag.disposeTarget(), true);
			else onScopeDispose$1(() => frag.scheduleTargetDispose(), true);
			if (!isHydrating$1) {
				if (_insertionParent) insert(frag, _insertionParent, _insertionAnchor, parentSuspense);
			} else frag.hydrate();
			return frag;
		...
		onScopeDispose$1(() => isInteropEnabled && currentUnmountSuspense !== void 0 ? unmountComponent(instance, void 0, currentUnmountSuspense) : unmountComponent(instance), true);
...
function createPlainElement(comp, rawProps, rawSlots, isSingleRoot, once) {
	...
	if (rawSlots) {
		...
		if (isHydrating$1) {
			...
			if (rawSlots.$ && !child) child = el.appendChild(markHydrationAnchor(!!(process.env.NODE_ENV !== "production") ? /* @__PURE__ */ createComment("") : /* @__PURE__ */ createTextNode()));
	...
	setup(props, { slots, expose }) {
		...
		const keepAliveCtx = {
			...
			runBranchRender(frag, fn, useScope, removePrevious) {
				...
				frag.scope = useScope ? cachedScope || new EffectScope$1() : void 0;
...
function isolateSlotSources(rawSlots) {
	...
	const committedSources = shallowReactive$1([]);
	let hasFunctionSource = false;
	for (let i = 0; i < dynamicSources.length; i++) {
		const source = dynamicSources[i];
		if (isFunction(source)) {
					...
					if (isVNode(slotContent)) {
						...
						if (slotResolutionState.activeFallback && slotContentValid && transition) {
							if (mode === "out-in") {
								...
								const enterHooks = extend$1({}, transition);
...
function createIf(condition, b1, b2, flags = 1) {
	...
	if (flags & 16) {
		...
		const dynamicFragment = new DynamicFragment(isHydrating$1 || !!(process.env.NODE_ENV !== "production") ? "if" : void 0, keyed, false, trackSlotBoundary, trackSlotBoundary ? () => {
...
function createKeyedFragment(key, render, trackSlotBoundary = false) {
	...
	const frag = new DynamicFragment(!!(process.env.NODE_ENV !== "production") ? "keyed" : void 0, true, true, trackSlotBoundary, trackSlotBoundary ? () => {
...
function createDynamicComponent(getter, rawProps, rawSlots, flags = 0) {
	...
	if (!isHydrating$1) resetInsertionState();
	const hydrationCursor = isHydrating$1 ? captureHydrationCursor() : null;
	const frag = new DynamicFragment(isHydrating$1 || !!(process.env.NODE_ENV !== "production") ? "dynamic-component" : void 0, false, true, slotRoot, slotRoot ? () => {
...
function withVaporDirectives(node, dirs) {
	...
	function applyDirectives() {
		...
		directiveScope = new EffectScope$1(true);
