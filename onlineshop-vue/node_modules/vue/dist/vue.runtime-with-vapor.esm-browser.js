...
function enterAsyncHydration(node) {
	...
	if (!isHydratingEnabled$1) setIsHydratingEnabled$1(true);
	setIsHydrating(true);
	...
	return () => {
		...
		if (!asyncHydrationIsEnabled) setIsHydratingEnabled$1(false);
	};
}
let adoptTemplate;
...
function applyGroupTransitionHooks(block, props, state, instance, updateHooks) {
	...
	for (let i = 0; i < children.length; i++) {
		const child = children[i];
		if (isValidTransitionBlock(child)) if (child.$key != null) child.$transition = resolveTransitionHooks$1(child, props, state, instance);
